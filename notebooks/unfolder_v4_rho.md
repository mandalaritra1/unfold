...
for stale_name in ["run_button", "open_button", "master_output", "master_controls"]:
...
master_controls = widgets.VBox([
    status_html,
    widgets.HBox([run_button, open_button]),
    master_output,
])
...
```python
unfolder_groomed.plot_unfolded_fancy()
```

```python
unfolder_groomed.plot_systematic_fraction()
```
...
edges = unfolder.bins.gen_rho_edges_by_pt[0][1:]
for i in range(1,4):
...
```python
for i in range(10):
    plt.stairs(np.abs(unfolder.mosaic_2d_jk_list[i][12:24]), unfolder.bins.reco_rho_edges_by_pt[2], label = int(i))
plt.legend()
plt.xlim(-2.5, 0)
plt.xlabel(rf"Ungroomed $\log_{10}(\rho^2)$")
plt.ylabel("# Events")
hep.cms.label("Internal", data=True, lumi=138)
plt.show()
for i in range(10):
    plt.stairs(np.abs(unfolder.y_unf_jk_list[i][6:12]), unfolder.bins.gen_rho_edges_by_pt[2],label = int(i))
plt.xlim(-2.5, 0)
plt.xlabel(rf"Ungroomed $\log_{10}(\rho^2)$")
plt.ylabel("# Events")
plt.legend()
...
for i in range(3):
    plt.subplot(2, 2, i+1)
    m2d_h = output_pythia['response_matrix_rho_u'].project('ptreco', 'mpt_reco', 'mpt_gen')[i, :, :]
    if i == 0:
        m2d_h = rebin_hist(m2d_h, 'mpt_gen', [ -2.5, -2, -1.5, -1, -0.5, 0])
        m2d_h = rebin_hist(m2d_h, 'mpt_reco' , [ -2.5, -2, -1.5, -1, -0.5, 0])
    if i == 1:
        m2d_h = rebin_hist(m2d_h, 'mpt_gen', [ -2.5, -2, -1.5, -1, -0.5, 0])
        m2d_h = rebin_hist(m2d_h, 'mpt_reco' , [ -2.5, -2, -1.5, -1, -0.5, 0])
    if i == 2:
    ...
    #draw a horizontal line at 0.5
    plt.axhline(0.5, color='gray', ls='--')
    plt.xlabel(r'Ungroomed Jet $\log_{10}(\rho^2)$')
    plt.ylabel('Purity/Stability')
...
for i in range(3):
    plt.subplot(2, 2, i+1)
    m2d_h = output_pythia['response_matrix_rho_g'].project('ptreco', 'mpt_reco', 'mpt_gen')[i, :, :]
    if i == 0:
        m2d_h = rebin_hist(m2d_h, 'mpt_gen', [-10, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0])
        m2d_h = rebin_hist(m2d_h, 'mpt_reco' , [-10, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0])
    if i == 1:
        m2d_h = rebin_hist(m2d_h, 'mpt_gen', [-10, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0])
        m2d_h = rebin_hist(m2d_h, 'mpt_reco' , [-10, -4.5, -4, -3.5, -3, -2.5, -2, -1.5, -1, -0.5, 0])
    if i == 2:
    ...
    #draw a horizontal line at 0.5
    plt.axhline(0.5, color='gray', ls='--')
    plt.xlabel(r'Groomed Jet $\log_{10}(\rho^2)$')
    plt.ylabel('Purity/Stability')
    hep.cms.label("Internal", data=False, rlabel = r"138 fb$^{-1}$")
...
for ipt in range(4):
    ...
    if not groomed:
        ax_ratio.set_xlabel(r"Ungroomed Jet $\log_{10}(\rho^2)$")
        ax_ratio.set_xlim(-2.5, 0)
    else:
        ax_ratio.set_xlabel(r"Groomed Jet $\log_{10}(\rho^2)$")
        ax_ratio.set_xlim(-4.5, 0)
