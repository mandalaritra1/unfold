...
from unfold.tools.unfolder_core import Unfolder, MASS_SPEC
...
def save_mode_artifacts(mode_name, unfolder_instance):
    np.savez(
        REPO_ROOT / "outputs" / f"unfolding_mosaic_mass_{mode_name}.npz",
        mosaic=unfolder_instance.mosaic,
    )
    np.savez(
        REPO_ROOT / "outputs" / f"unfolding_mosaic_2d_mass_{mode_name}.npz",
        mosaic_2d=unfolder_instance.mosaic_2d,
    )
...
def validate_master_run_state():
    ...
    if missing_modes:
        raise RuntimeError(
            f"Master run finished without creating: {', '.join(missing_modes)}"
        )
...
run_button = widgets.Button(
    description="Run groomed + ungroomed", button_style="primary", icon="play"
)
open_button = widgets.Button(
    description="Open mass gallery", button_style="success", icon="external-link"
)
print_logs_checkbox = widgets.Checkbox(
    value=False, description="Print full logs", indent=False
)
...
def handle_run(btn):
    ...
    with master_output:
        ...
        try:
            ...
            display(
                Markdown(
                    'Finished. Default `unfolder` now points to `unfolder_ungroomed`. Use `use_unfolder("groomed")` if you want the groomed object in later cells.'
                )
            )
            ...
            display(
                Markdown(
                    "Master run failed before creating the expected unfolding objects."
                )
            )
...
for i in range(1, 4):
    ...
    plt.sca(ax2)
    plt.stairs(
        np.abs(r1 - r2) / np.abs(r2),
        edges,
        label="Relative Difference (Rewighted Pythia)",
    )
...
for i in range(10):
    plt.stairs(
        np.abs(unfolder.mosaic_2d_jk_list[i][12:24]),
        unfolder.bins.reco_mass_edges_by_pt[2],
        label=int(i),
    )
plt.legend()
plt.xlim(0, 215)
...
for i in range(10):
    plt.stairs(
        np.abs(unfolder.y_unf_jk_input_list[i][6:12]),
        unfolder.bins.gen_mass_edges_by_pt[2],
        label=int(i),
    )
plt.xlim(0, 215)
plt.xlabel("Ungroomed Jet Mass (GeV)")
...
for i in range(4):
    plt.subplot(2, 2, i + 1)
    m2d_h = output_pythia["response_matrix_u"].project("ptreco", "mreco", "mgen")[
        i, :, :
    ]
...
for i in range(4):
    plt.subplot(2, 2, i + 1)
    m2d_h = output_pythia["response_matrix_g"].project("ptreco", "mreco", "mgen")[
        i, :, :
    ]
    m2d_h = rebin_hist(m2d_h, "mgen", bins.gen_mass_edges_by_pt[i])
    m2d_h = rebin_hist(m2d_h, "mreco", bins.reco_mass_edges_by_pt[i])
...
for ipt in range(4):
    ...
    resp_matrix_4d = (
        ...
        if not groomed
        else output_pythia["response_matrix_g"]
    )
    resp_matrix_4d_herwig = (
        output_herwig["response_matrix_u"]
        if not groomed
        else output_herwig["response_matrix_g"]
    )
    input_data = (
        output_data["ptjet_mjet_u_reco"]
        if not groomed
        else output_data["ptjet_mjet_g_reco"]
    )
    ...
    mc_values = resp_matrix_4d.project("systematic", "ptreco", "mreco")[
        "nominal", ipt, :
    ].values()
    mc_values_norm = mc_values / mreco_width / mc_values.sum()
    mc_stat = (
        resp_matrix_4d.project("systematic", "ptreco", "mreco")[
            "nominal", ipt, :
        ].variances()
        ** 0.5
    )
    ...
    mc2_values = resp_matrix_4d_herwig.project("systematic", "ptreco", "mreco")[
        "nominal", ipt, :
    ].values()
    mc2_values_norm = mc2_values / mreco_width / mc2_values.sum()
    mc2_stat = (
        resp_matrix_4d_herwig.project("systematic", "ptreco", "mreco")[
            "nominal", ipt, :
        ].variances()
        ** 0.5
    )
    ...
    ratio = np.divide(
        np.abs(mc_values_norm),
        np.abs(data_values_norm),
        out=np.ones_like(mc_values),
        where=data_values != 0,
    )
    ratio2 = np.divide(
        np.abs(mc2_values_norm),
        np.abs(data_values_norm),
        out=np.ones_like(mc_values),
        where=data_values != 0,
    )
    ...
    ratio1_err = ratio1 * np.sqrt(
        (data_stat_norm / data_values_norm) ** 2 + (mc_stat_norm / mc_values_norm) ** 2
    )
...
ptjet_mjet_herwig_gen = output_herwig["response_matrix_u"][
    {"systematic": "herwig"}
].project("ptgen", "mgen")

misses = output_pythia["misses_u"].project("ptgen", "mgen")
ptjet_mjet_gen = output_pythia["response_matrix_u"][{"systematic": "nominal"}].project(
    "ptgen", "mgen"
)
...
for i in range(len(fakes.values())):
    ...
    true_miss_values_herwig = (
        misses_herwig[i, :].values() - ptjet_mjet_herwig_gen[0, :].values()
    )

    miss_rate = misses[i, :].values() / (
        ptjet_mjet_gen[i, :].values() + misses[i, :].values()
    )
    miss_rate_herwig = misses_herwig[i, :].values() / (
        ptjet_mjet_herwig_gen[i, :].values() + misses_herwig[i, :].values()
    )
    ...
    fake_fraction_herwig = (
        fakes_herwig[i, :].values() / ptjet_mjet_herwig[i, :].values()
    )

    # hep.histplot(1 - fake_fraction_herwig, mreco_edge, label = '1 - Fakes Herwig', color = 'red', ls = '--')
    hep.histplot(
        1 - fake_fraction, mreco_edge, label="1 - Fakes Pythia", color="blue", ls="-"
    )
...
ptjet_mjet_herwig_gen = output_herwig["response_matrix_g"][
    {"systematic": "herwig"}
].project("ptgen", "mgen")

misses = output_pythia["misses_g"].project("ptgen", "mgen")
ptjet_mjet_gen = output_pythia["response_matrix_g"][{"systematic": "nominal"}].project(
    "ptgen", "mgen"
)
