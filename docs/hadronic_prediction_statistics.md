# Hadronic prediction statistics

The hadronic runner now propagates MC prediction statistics through its
...
them to the prediction-statistics functions. The new hadronic mode consumes
...
- `src/unfold/tools/hadronic_vincia.py`: Vincia covariance, error bars, attachment
  and artifact export.
- `scripts/run_hadronic_unfolding.py`: opts into the method, changes the run
...
MPLBACKEND=Agg PYTHONPATH="$PWD/src:$PYTHONPATH" .venv/bin/python \
  ...
  --output outputs/hadronic/prediction_statistics_new
```

The 2026-09-08 output is in
`outputs/hadronic/prediction_statistics_2026-09-08_v2/`, with 16 PNG/PDF
