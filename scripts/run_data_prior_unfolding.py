...
This is Jennifer's prior-dependence check using already-produced weighted
PYTHIA histograms: unfold the same data once with the nominal PYTHIA rho
response and once with a PYTHIA response whose gen-level rho prior has been
reweighted toward the unfolded data.  The weighted input is a full producer
pickle, not an event-level correction.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--weighted-mc",
        type=Path,
        default=Path("/Users/aritra/Downloads/data_prior_weighted_pythia_all.pkl"),
        help="Producer pickle containing the data-prior-weighted PYTHIA rho histograms.",
    )
    parser.add_argument(
        "--tag",
        default="original",
