#!/usr/bin/env python3
"""ARC round-3 OP1/WS2: model closure vs the PS/HAD variations, for the AN.

Round-3 rerun of ``study_model_closure_arc_r2.py`` with two changes required
by the reviewers' wording ("in the case of reweighted samples, as the same
statistical data set is used, only modelling uncertainties should be used"):

* the comparison band is the MODELLING uncertainty of the measurement (the
  two-leg PS/HAD envelope, ``normalized_results[i]['model_unc_frac']``),
  not the statistical band -- the variations are reweighted PYTHIA, so the
  statistical fluctuations of pseudo-data and response largely cancel;
* the default tag is the publication candidate
  (``jmsjmr_unity_groomed400_floor3``) and the run uses ``do_syst=True`` so
  the model envelope is available for the band.

Everything below the band choice is the round-2 construction unchanged.

Round-2 rationale (still applies):
...
Usage:
  ...
  .venv/bin/python scripts/study_model_closure_arc_r2.py [tag]
"""
...
def run_mode(groomed, tag):
    ...
    # ARC round-3 caveat: reweighted samples share the statistical data set,
    # so the closure residuals are judged against the MODELLING band only.
    band_frac = [
    ...
    for source in SOURCES:
        ...
        for i, edges in enumerate(uf.gen_edges_by_pt):
            ...
            with np.errstate(divide="ignore", invalid="ignore"):
                ...
                # |u_n|: unfolded bins can be negative (buffer/edge bins);
                # the fractional error is a magnitude either way.
    ...
    _plot(uf, residuals, errors, band_frac, mode)
...
def _plot(uf, residuals, errors, band_frac, mode):
    hep.style.use("CMS")
    ...
    for i in published:
        ...
        plt.fill_between(edges, -np.append(band, band[-1]),
                         ...
                         label=r"Modelling unc. (PS $\oplus$ HAD)")
        if visible.any():
        ...
        for ext in ("pdf", "png"):
            fig.savefig(OUT_DIR / f"model_closure_r3_{mode}_{i}.{ext}",
                        bbox_inches="tight", pad_inches=0.1)
        plt.close(fig)
    print(f"  saved per-pT figures -> {OUT_DIR}/model_closure_r3_{mode}_*.pdf")
