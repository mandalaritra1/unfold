...
|---|---|
| `hadronic_plot_book.py` | the dijet / trijet plot book PDF from the four `original` run manifests |
| `hadronic_slide_deck.py` | slide deck built from the plot book and the combined figures |
| `combined_channels_rho.py` | Z+jet / dijet / trijet overlays from the saved artifacts |
| `datamc/` | reco-level data vs MC validation figures for the AN (Z+jet and hadronic), theory overlays, per-systematic reco variations |
