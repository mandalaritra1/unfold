...
# Universal switch for the provenance stamp on every figure (engine plots and
# the scripts alike).  ``unfold run --no-stamp`` or ``UNFOLD_NO_STAMP=1`` turns
# it off for publication figures.
STAMP_ENABLED = os.environ.get("UNFOLD_NO_STAMP", "") not in ("1", "true", "yes")


def set_stamp(enabled):
    """Turn the provenance stamp on or off for this process and its children."""
    global STAMP_ENABLED
    STAMP_ENABLED = bool(enabled)
    if enabled:
        os.environ.pop("UNFOLD_NO_STAMP", None)
    else:
        os.environ["UNFOLD_NO_STAMP"] = "1"
...
def stamp_figure(fig, inputs="n/a", repo="unfold", date=None):
    ...
    Off when ``STAMP_ENABLED`` is false (``set_stamp(False)``, ``--no-stamp``).
    """
