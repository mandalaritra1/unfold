"""Input discovery and schema adaptation for the 2018 dijet/trijet rho pickles."""
...
from unfold.histmath import rebin_hist
...
def to_binning(binning: RhoAnalysisBinning):
    """Convert the channel binning table into the shared ``Binning``."""
    from unfold.binning import Binning

    return Binning(
        pt_edges=tuple(binning.pt_edges),
        reco_edges=tuple(binning.rho_edges),
        gen_edges=tuple(binning.rho_edges_gen),
        reco_edges_by_pt=tuple(tuple(e) for e in binning.reco_rho_edges_by_pt),
        gen_edges_by_pt=tuple(tuple(e) for e in binning.gen_rho_edges_by_pt),
    )
