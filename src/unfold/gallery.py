...
def card_html(path: Path, root: Path, dpi: int) -> str:
    ...
    return f"""Static HTML gallery of the figures in an output directory (PDFs get PNG previews)."""
...
def build_gallery(root, output=None, dpi=PDF_DPI, title=None):
    """Write <root>/index.html with PDF previews cached under <root>/_previews."""
    root = Path(root).resolve()
    output = Path(output).resolve() if output else root / "index.html"
    tag = root.name if root.parent.name == "rho" else None
    title = title or (f"rho plot gallery: {tag}" if tag else "rho plot gallery")
    build_html(root=root, output=output, dpi=dpi, page_title=title, hero_title=title)
    return output


def main() -> None:
    args = parse_args()
    build_gallery(args.root, args.output, args.dpi, args.title)
